...
func TestReadLines(t *testing.T) {
	...
	assert.Equal(t, uint64(8417855), event.Actor.Id)
	assert.Equal(t, "dwtechgogators", event.Actor.Login)
	assert.Equal(t, uint64(48892041), event.Repo.Id)
	...
	event = <-c
	assert.Equal(t, "PushEvent", event.Type)
	assert.Equal(t, uint64(4620546), event.Actor.Id)
	assert.Equal(t, "webbam46", event.Actor.Login)
	assert.Equal(t, uint64(46007818), event.Repo.Id)
	...
	event = <-c
	assert.Equal(t, "CreateEvent", event.Type)
	assert.Equal(t, uint64(11765156), event.Actor.Id)
	assert.Equal(t, "murraykj", event.Actor.Login)
	assert.Equal(t, uint64(42083632), event.Repo.Id)
	...
	event = <-c
	assert.Equal(t, "PushEvent", event.Type)
	assert.Equal(t, uint64(11095732), event.Actor.Id)
	assert.Equal(t, "JackyChiu", event.Actor.Login)
	assert.Equal(t, uint64(48559800), event.Repo.Id)
	...
	event = <-c
	assert.Equal(t, "PushEvent", event.Type)
	assert.Equal(t, uint64(2941391), event.Actor.Id)
	assert.Equal(t, "midfingr", event.Actor.Login)
	assert.Equal(t, uint64(48193506), event.Repo.Id)
	...
	event = <-c
	assert.Equal(t, "PushEvent", event.Type)
	assert.Equal(t, uint64(1759081), event.Actor.Id)
	assert.Equal(t, "christianfredh", event.Actor.Login)
	assert.Equal(t, uint64(30062860), event.Repo.Id)
	assert.Equal(t, "aptitud/aptitud.github.io", event.Repo.Name)
	assert.Equal(t, time.Date(2016, 1, 1, 23, 0, 2, 0, utc), event.CreatedAt)
	assert.Equal(t, uint64(2202349), event.Org.Id)
	assert.Equal(t, "aptitud", event.Org.Login)

	event = <-c
	assert.Equal(t, "PushEvent", event.Type)
	assert.Equal(t, uint64(533180), event.Actor.Id)
	assert.Equal(t, "scriptzteam", event.Actor.Login)
	assert.Equal(t, uint64(41249359), event.Repo.Id)
	...
	event = <-c
	assert.Equal(t, "PushEvent", event.Type)
	assert.Equal(t, uint64(6753598), event.Actor.Id)
	assert.Equal(t, "bradleyboehmke", event.Actor.Login)
	assert.Equal(t, uint64(48668209), event.Repo.Id)
